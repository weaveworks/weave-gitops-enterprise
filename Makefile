...
kerberos/cmd/wk-kerberos/wk-kerberos: $(shell find kerberos/cmd/wk-kerberos/ -type f -name '*.go')
	CGO_ENABLED=0 GOARCH=amd64 go build -o kerberos/cmd/wk-kerberos/wk-kerberos ./kerberos/cmd/wk-kerberos
...
kerberos/cmd/k8s-krb5-server/server: kerberos/cmd/k8s-krb5-server/*.go
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "-X main.version=$(VERSION)" -o kerberos/cmd/k8s-krb5-server/server ./kerberos/cmd/k8s-krb5-server
