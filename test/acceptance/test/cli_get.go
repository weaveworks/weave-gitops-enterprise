		...
		Context("[CLI] When profiles are available in the management cluster", func() {
			It("Verify gitops can list profiles from profile repository", func() {
				...
				By("Then gitops lists profiles", func() {
					Eventually(stdOut).Should(MatchRegexp(`cert-manager\s+A Weaveworks Helm chart for the Certificate Profile[,.\d\w\s]+0.0.8,0.0.7`))
					Eventually(stdOut).Should(MatchRegexp(`weave-policy-agent\s+A Weaveworks Helm chart for Kubernetes to configure the policy agent[,.\d\w\s]+0.4.0`))
					Eventually(stdOut).Should(MatchRegexp(`metallb\s+A Weaveworks Helm chart for a network load-balancer implementation[,.\d\w\s]+0.0.2,0.0.1`))
				})
			})
		})
