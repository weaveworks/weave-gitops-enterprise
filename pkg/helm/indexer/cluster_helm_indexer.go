...
type ClusterHelmIndexerTracker struct {
	...
	Namespace       string
	ClusterWatchers map[clustersmngr.Cluster]*controller.HelmWatcherReconciler
}
...
func NewClusterHelmIndexerTracker(c cache.Cache, rm helm.RepoManager, ns string) *ClusterHelmIndexerTracker {
	return &ClusterHelmIndexerTracker{
		...
		RepoManager:     rm,
		Namespace:       ns,
		ClusterWatchers: make(map[clustersmngr.Cluster]*controller.HelmWatcherReconciler),
	}
}

func (c *ClusterHelmIndexerTracker) newIndexer(c clustersmngr.Cluster) error {
