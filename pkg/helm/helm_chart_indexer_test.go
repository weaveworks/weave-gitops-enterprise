...
func TestGetProfileValues(t *testing.T) {
	...
	if err := indexer.AddChart(context.TODO(), "redis", "1.0.1", "chart", []byte("hi: there"),
...
func TestListAvailableVersionsForProfile(t *testing.T) {
	...
	if err := indexer.AddChart(context.TODO(), "redis", "1.0.2", "chart", nil,
		nsn(ManagementClusterName, ManagementClusterNamespace),
		objref("HelmRepository", "", "bitnami-charts", "team-ns")); err != nil {
		t.Fatal(err)
	}
	if err := indexer.AddChart(context.TODO(), "nginx", "1.0.3", "chart", nil,
		nsn(ManagementClusterName, ManagementClusterNamespace),
		objref("HelmRepository", "", "bitnami-charts", "team-ns")); err != nil {
		t.Fatal(err)
	}
