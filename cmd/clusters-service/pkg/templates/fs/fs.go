...
func (f FSLibrary) flavoursFromDir(dir, version string) ([]*templates.Flavour, error) {
	...
	for _, v := range dirs {
		if !v.IsDir() {
			t, err := templates.ParseFileFromFS(f.FS, filepath.Join(dir, v.Name()))
			if err != nil {
				return nil, fmt.Errorf("failed to parse: %w", err)
			}

			params, err := templates.ParamsFromSpec(t.Spec)
			if err != nil {
				return nil, err
			}
			found = append(found, &templates.Flavour{
