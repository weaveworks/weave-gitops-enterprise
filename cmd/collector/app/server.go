...
func (s *Server) Start(ctx context.Context) error {
	if err := s.arc.Start(ctx); err != nil {
...
func NewServer(ctx context.Context, opts ServerOpts) (*Server, func() error, error) {
	...
	// create collectors
	rulesCollector, err := accesscollector.NewAccessRulesCollector(opts.RemoteStore, optsCollector)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to create access rules collector: %w", err)
	}

	objsCollector, err := objectscollector.NewObjectsCollector(opts.RemoteStore, optsCollector)
	if err != nil {
